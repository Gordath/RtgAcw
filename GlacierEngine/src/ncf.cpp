...
int NCF::parse()
{
	...
	while (getline(in, input))
	{
		...
		NcfUtil::String::split(input, line, comment, '#'); // Strip comments.
		NcfUtil::String::trim(line); // Trim spaces and unused characters.
		...
		else if (line.length() > 2 && (line.find('=') != std::string::npos))
		{
			...
			NcfUtil::String::split(line, name, value, '=');

			// Trim the name and its value from unused characters and spaces.
			NcfUtil::String::trim(name);
			NcfUtil::String::trim(value);
		...
		else if((line.length() > 1) && (line.find('%') != std::string::npos))
		{
			...
			if (!com.compare("%include"))
			{
				...
				NcfUtil::String::path_comp(m_p_filepath, base, file);
...
void NCF::expand_symbol(std::map<std::string, std::string> &symbols, std::string &s)
{
	...
	do {
		...
		// Extract reference.
		NcfUtil::String::extract(source,search,'<','>');
		...
		for (std::map<std::string, std::string>::iterator it = symbols.begin(); it != end; ++it) {
			...
			if (!pos)
			{
				NcfUtil::String::replace_first_of(s, search, it->second);
#ifdef CONFIG_DEBUG
...
{
	if (count_properties() == 0) return nullptr;
...
{
	if (count_groups() == 0) return nullptr;
